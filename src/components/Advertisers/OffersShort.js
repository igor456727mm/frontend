...
class Countries extends Component {
  ...
  render() {
    ...
    if(country_data.type == 'list') {
      return (
        <div className="offers__table-countries" style={{ wordBreak: 'normal' }}>
        ...
        <div className="offers__table-countries" style={{ wordBreak: 'normal' }}>
          ...
          {country_data.except && (
            <div className="offers__table-countries-trought">
              {this.renderCountries(country_data.except, limit)}
              {country_data.except && country_data.except.length > limit && (
                <Popover
                  content={(<div className="offers__table-countries" style={{ textDecoration: "line-through" }}>{this.renderCountries(country_data.except)}</div>)}
                  trigger="click"
                  visible={isVisible}
                  onVisibleChange={this._toggle}>
...
class OffersShort extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        ...
        {
          title: 'Тип оплаты (название/знач./знач.)',
          dataIndex: '',
          render: (text, row) => {
            const { actions } = text
            const getPlainActions = actions && Array.isArray(actions) && actions.length > 0 && actions.reduce((acc, item) => {
              ...
              switch (pay_type) {
                ...
                case 'fix':
                  ...
                  condition = <span>{name} / Стоимость <span>{price}$</span> / Комиссия <span>{commission}$</span></span>
                  return actions.length === 1 ? condition : <div>{condition};<br/>{acc}</div>
                case 'revshare':
                  ...
                  condition = <span>{name} / Ревшара сайта: <span>{site_revshare_percent}%</span> / Ревшара вебмастера: <span>{revshare_percent}%</span></span>
                  return actions.length === 1 ? condition : <div>{condition};<br/>{acc}</div>
              }

            }, '')
            return <div style={{ wordBreak: 'normal' }}>{getPlainActions}</div>;
          },
        },
        {
          title: 'GEO',
